...
function highlightRecord(record, formattedRecord) {
...
export class PatternLayout {
  ...
  format(record) {
    ...
    if (this._highlight) {
      highlightRecord(record, formattedRecord);
    }

    return this._pattern.replace(PATTERN_REGEX, match =>
      formattedRecord.get(match)
    );
  }
}
