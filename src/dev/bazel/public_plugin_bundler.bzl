...
def _public_plugin_bundler_impl(ctx):
  ...
  run_node(
    ...
    outputs = outputs,
    mnemonic = "BundlePublicPlugin",
    progress_message = "Bundling public plugin %s" % package_dir.short_path,
    executable = "_bundler",
  )
...
public_plugin_bundler = rule(
  implementation = _public_plugin_bundler_impl,
  attrs = {
...
#    "package_name": attr.string(),
    ...
    "_bundler": attr.label(
      doc = "Target that executes the public plugins bundler binary",
      executable = True,
      cfg = "host",
      default = Label("//packages/kbn-optimizer:_bin-optimizer-cli"),
    ),
  },
)
